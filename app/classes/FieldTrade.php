...
class FieldTrade {
	...
	public function download(){
		$activity = Activity::find($this->activity);
		$excel = $this->generate($activity);
		$excel->download('xls');
	}
	...
	public function save(){
		...
		$excel->store('xls',storage_path().$filepath);
	}
			...
			if(!empty($tradedeal)){
				...
				return Excel::create($file_name. ' BBFEE', function($excel) use($activity){
				    ...
				    $excel->sheet('ALLOCATIONS', function($sheet) use ($activity) {
						...
						$p_col = $col - 1;
						$n_col = $p_col+1;
						$sheet->setCellValueByColumnAndRow($col,1, 'BUDGET');
						$sheet->setCellValueByColumnAndRow($col,2, 'TOTAL BUDGET');
						$budget_col = $col;
						

						$sheet->getStyle('A1:'.\PHPExcel_Cell::stringFromColumnIndex($col).'2')
							->getFont()
							->setBold(true);
						
						// Set background color for a specific cell
						$sheet->getStyle('A2:'.\PHPExcel_Cell::stringFromColumnIndex($n_col).'2')->applyFromArray(array(
						    'fill' => array(
						        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
						...
						$sheet->cells(\PHPExcel_Cell::stringFromColumnIndex($d_col).'1:'.\PHPExcel_Cell::stringFromColumnIndex($n_col).'1', function($cells) {
							$cells->setFontColor('#ffffff');
						});

						$sheet->getStyle(\PHPExcel_Cell::stringFromColumnIndex($n_col).'1:'.\PHPExcel_Cell::stringFromColumnIndex($n_col).'1')
							->applyFromArray(array(
						    'fill' => array(
						        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
						        'color' => array('rgb' => '68993A')
						    )
						));

						

						$sheet->getStyle(\PHPExcel_Cell::stringFromColumnIndex($d_col).'1:'.\PHPExcel_Cell::stringFromColumnIndex($n_col).'1')->applyFromArray($style);
								...
								if($last_distributor == $alloc->sold_to_code){
									...
									if($last_site == $alloc->plant_code){
										...
										foreach ($col_pre as $col) {
											...
											$sum_budget = "=SUM(".\PHPExcel_Cell::stringFromColumnIndex($budget_col).$first_row.":".\PHPExcel_Cell::stringFromColumnIndex($budget_col).$last_row.")";
											$sheet->setCellValueByColumnAndRow($budget_col,$row,$sum_budget);
										...
										$sheet->getStyle('D'.$row.':'.\PHPExcel_Cell::stringFromColumnIndex($budget_col).$row)->applyFromArray(array(
										    'fill' => array(
										        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
										        'color' => array('rgb' => 'DAEBF8')
										    )
									...
									if($tradedeal->non_ulp_premium){
										$sheet->setCellValueByColumnAndRow($budget_col,$row, "non-ULP");
									}else{
										$sheet->setCellValueByColumnAndRow($budget_col,$row, $alloc->computed_cost);
									}

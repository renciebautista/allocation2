	...
	<div class="col-lg-12">
		<div class="form-group">
			...
			@if(count($activityIdList) > 1)
				@if(!$id_index)
					{{ HTML::linkAction('ActivityController@preapproveedit','Previous', array('id' => $activity->id), array('class' => 'btn btn-primary disabled')) }}
					{{ HTML::linkAction('ActivityController@preapproveedit','Next', array('id' => $activityIdList[1]), array('class' => 'btn btn-primary')) }}
				@else
					...
					@else
						<?php 
							$id = $id_index - 1;
						?>
						{{ HTML::linkAction('ActivityController@preapproveedit','Previous', array('id' => $activityIdList[$id]), array('class' => 'btn btn-primary')) }}
					@endif
					...
					@if($activityIdList[$last_cnt - 1] == $activity->id)
						{{ HTML::linkAction('ActivityController@preapproveedit','Next', array('id' => $activity->id), array('class' => 'btn btn-primary disabled')) }}
					@else
						<?php 
							$id = $id_index + 1;
						?>
						{{ HTML::linkAction('ActivityController@preapproveedit','Next', array('id' => $activityIdList[$id]), array('class' => 'btn btn-primary')) }}
					@endif
				@endif
			@endif
							...
							<div class="allocation_total table-responsive">
								<table class="table table-condensed table-bordered display compact datatable">
									<thead>
										...
										<?php $first_index = 1; ?>
										@foreach($scheme->sobgroups as $group)
										<tr class="sob-percent">
											<th colspan="4" >
												<span class="pull-right">{{ $group->sobgroup }}</span>
											</th>
											<?php $total = 0; ?>
											@foreach($scheme->sob_header[$group->id] as $key => $header)
											<th class="alloc_per">

...
function duration(value){
	$.ajax({
		type: "GET",
		url: hostname + "/activitytype/"+value+"/network/totalduration",
		success: function(msg){
...
$("#implementation_date").on("dp.change",function (e) {
	$.ajax({
		type: "GET",
		url: hostname + "/activitytype/"+$('#activity_type').val()+"/network/totalduration?sd="+moment($('#implementation_date').val()).format('DD-MM-YYYY'),
		success: function(msg){
			$('#lead_time').val(msg.days);
			$('#implementation_date').val(msg.end_date);
			$('#download_date').val(msg.start_date)
